    ...
    try:
        query = f"""
            SELECT
                ...
                CONVERT_TZ(u.creation_time, 'UTC', 'Europe/Athens')             AS createdtime,
                u.last_update_time                                          AS modifiedtime,
                u.fns_status                                                AS questionnaire_completed,
                u.user_type                                                 AS client_category,
                CONVERT_TZ(uair.qualification_time, 'UTC', 'Europe/Athens')     AS client_qualification_date,
...
def get_accounts_full():
    """Yields DataFrames in chunks using keyset pagination on u.id (fresh connection per chunk)."""
    last_id = 0
    while True:
        conn = _get_connection()
        try:
        ...
        last_id = int(df["accountid"].max())
        yield df


def get_accounts_by_qual_date(from_date: str) -> pd.DataFrame:
    """Fetch accounts where qualification_time (UTC+2) >= from_date. Used for targeted re-sync."""
    conn = _get_connection()
    try:
        query = f"""
            SELECT
            ...
            LEFT JOIN crmdb.aggregated_user_data aud ON (u.id = aud.user_id AND 0 <> aud.latest)
            LEFT JOIN crmdb.app ON u.registration_app = crmdb.app.id
            WHERE CONVERT_TZ(uair.qualification_time, 'UTC', 'Europe/Athens') >= '{from_date}'
              AND u.id IS NOT NULL
              AND u.id != ''
              AND u.is_test = 0
        """
        ...
        query = f"""
            SELECT * FROM (
                SELECT
                    ...
                    CONVERT_TZ(bb.decision_time, 'UTC', 'Europe/Athens')                AS confirmation_time,
                    ...
                    CONVERT_TZ(bb.creation_time, 'UTC', 'Europe/Athens')                AS created_time,
