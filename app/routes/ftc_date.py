...
async def ftc_date_api(
    ...
    sql = (
        ...
        "           MAX(CASE WHEN t.transaction_type_name = 'Withdrawal' THEN 1 ELSE 0 END) AS has_withdrawal\n"
        "    FROM transactions t\n"
        ...
        "    FROM (\n"
        "        SELECT p.login, p.notional_value\n"
        ...
        "           COALESCE(tx.has_withdrawal, 0) AS is_withdrawaler,\n"
